...
| `bootstrap_chunk_corpus.py` | Chunkea el corpus completo existente (registry, News x2 variantes parrafo/articulo, Comments, Digest) en texto listo para embeder, sin llamar a ningun modelo todavia -- paso 1 del bootstrap de Fase 2 (embedding), ver architecture_canon.md | 2026-08-20, dry-run validado; `--apply` corrido el mismo dia en la sesion siguiente | Terminado y aplicado -- 5 archivos escritos a `chunks/<fuente>/bootstrap.json` en S3 (913 registry / 109,511 news_paragraph / 6,695 news_article / 629 comments / 10 digest chunks), verificado post-escritura con `aws s3 ls --recursive`. Dos bugs reales encontrados y corregidos en el dry-run antes de escribir: separador de parrafo real es `\n` no `\n\n`, y `comment_entity_id` no vive en el payload del comentario (requiere lookup al registry). Pasos 2 (embedding) y 3 (escritura a Pinecone) del bootstrap de Fase 2 siguen sin correr |
| `bootstrap_embed_corpus.py` | Convierte los chunks de `chunks/` en vectores llamando al modelo de embedding, con checkpointing incremental a `vectors/_checkpoints/` cada 2,000 chunks y resume real (salta partes ya escritas) -- paso 2 del bootstrap de Fase 2 | 2026-08-20/21, multiples intentos, **ninguno completo** | **BLOQUEADO, ver tech_debt.md "Phase 2 Embedding Bootstrap BLOCKED"**. Titan v2 eliminado del proyecto (sin batch API + cuota 600 req/min). Voyage completo su corpus pero sus checkpoints se borraron por error, y su free tier estaba en 56.5% consumido. Cohere v4 bloqueado por throttling sostenido (~15-20/min), hipotesis viva: el limite DIARIO de 8.1M tokens, ya al 65% ese dia. Solo existe 1 checkpoint parcial (`news_article_variant/cohere/part_00000.json`) |
