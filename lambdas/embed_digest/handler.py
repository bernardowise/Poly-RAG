"""
Poly-RAG embed_digest Lambda: embeds the current cycle's digest chunk into a
vector via Cohere Embed v4, ready for Fase 3 (store write, out of scope for
now per explicit user decision 2026-08-22). Fase 2, level 3 (embedding).

WHY THIS EXISTS
---------------
Ports the proven, real-corpus-tested embedding mechanism from
scripts/embed_corpus_slice.py (2026-08-21/22 -- see tech_debt.md, "Phase 2
Embedding Bootstrap", both corrections) into a per-cycle Lambda. Same token-
aware batching, same sliding-window rate governor, same MODEL_ID
(global.cohere.embed-v4:0, chosen after two real outages -- see tech_debt.md
for why bare on-demand and us. cross-region both hit independent daily
quotas), same checkpointing mechanism.

Digest is the simplest of the 4 sources to embed: exactly 1 chunk per cycle,
1 Bedrock request, done in seconds. No batching/pacing concern in practice at
this volume -- the machinery is ported anyway for consistency with the other
3 embed Lambdas and in case a future cycle's digest text grows.

SEQUENCING (level 3 is 4 Lambdas run in SEQUENCE, not parallel -- decided
2026-08-22): running the 4 embed Lambdas in parallel would mean 4 independent
processes competing for the same Cohere TPM ceiling without coordinating,
recreating the exact race condition that already caused the real News
double-invocation incident (see tech_debt.md, "Strict Ingestion Chaining").
Sequential means only one embed Lambda ever calls Bedrock at a time, so the
rate governor's view of "what's been sent in the last 60s" is always
accurate for the whole account, not just this one Lambda's slice of it.
Order (cheapest first, matching what was measured running these 4 sources by
hand on 2026-08-21/22): embed_digest -> embed_comments -> embed_registry ->
embed_news_article.

TRIGGER
-------
Invoked by chunk_digest (or, once the full chain is wired, by whichever
mechanism detects all 4 chunking Lambdas finished -- see
poly-rag-embed-orchestrator). Carries cycle_started_at.

OUTPUT
------
Writes s3://<bucket>/vectors/_checkpoints/digest/cohere/part_NNNNN.json, then
invokes embed_comments with the same cycle_started_at.
"""
...
NEXT_LAMBDA_NAME = os.environ.get("NEXT_LAMBDA_NAME", "poly-rag-embed-comments")
...
EMBED_DIM = 1536
# Cohere Embed v4 real confirmed pricing (see tech_debt.md, "Embedding Model
# Choice", verified 2026-08-20 via AWS pricing docs) -- input tokens only,
# this model has no separate output-token cost (embeddings, not generation).
...
def write_embedding_metric(cycle_started_at, tokens_in, latency_ms, chunk_count):
    """One row per Bedrock request (not per Lambda invocation) -- see
    tech_debt.md, "Day 6" entry, 2026-08-20 decision on per-source tracking.
    estimated_cost_usd computed from real confirmed pricing, not billed_units
    from the response (Cohere's embed response carries no meta/billing field,
    confirmed 2026-08-21 -- see tech_debt.md, embed_corpus_slice.py
    docstring)."""
