...
DIGEST_METRICS_LAMBDA_NAME = os.environ.get("DIGEST_METRICS_LAMBDA_NAME", "poly-rag-digest-metrics")
...
def lambda_handler(event, context):
    ...
    # Last stage of the embedding chain -- invoke digest_metrics, which sends
    # the cost/latency/tokens report email (split out 2026-08-22, see its own
    # docstring for why: this Lambda's job is embedding news_article, not
    # reporting -- those were only bundled together because this happened to
    # be the last stage). Runs regardless of whether THIS Lambda had new
    # chunks to embed (chunks may be empty if news_article's chunk file was
    # already fully covered), since the report covers the whole cycle's
    # Fase 1+2 activity, not just this Lambda's own work.
    lambda_client.invoke(
        FunctionName=DIGEST_METRICS_LAMBDA_NAME,
        InvocationType="Event",
        Payload=json.dumps({"cycle_started_at": cycle_started_at}),
    )
    result["digest_metrics_invoked"] = DIGEST_METRICS_LAMBDA_NAME
