"""
Poly-RAG embed_registry Lambda: embeds this cycle's NEW registry markets
(question+description) into vectors via Cohere Embed v4. Fase 2, level 3
(embedding), third in the sequential chain (see embed_digest handler.py
docstring for the full sequencing rationale -- 4 embed Lambdas run in
SEQUENCE, not parallel, to avoid multiple processes competing for the same
Cohere TPM ceiling).
...
Chunk input already scoped to just this cycle's new markets by
chunk_registry (see that handler's docstring for the first_seen >
cycle_started_at mechanism, corrected 2026-08-22 to not need any lookback
window math).

TRIGGER: invoked by embed_comments, carries cycle_started_at.
OUTPUT: writes vectors/_checkpoints/registry/cohere/part_NNNNN.json, then
invokes embed_news_article.
"""
...
NEXT_LAMBDA_NAME = os.environ.get("NEXT_LAMBDA_NAME", "poly-rag-embed-news-article")
